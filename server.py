...
def oracle():
    """Gemini AI tarot reading endpoint."""
    if not gemini_model:
        return jsonify({'error': 'GEMINI_API_KEY not configured. Set it as an environment variable.'}), 503
    ...
    try:
        if mode == 'single':
            ...
            future = cards[2]
            prompt = (
                f"You are a mystical tarot oracle performing a Past-Present-Future spread. "
                f"Past card: {past['name']} — {past['meaning']}. "
                f"Present card: {present['name']} — {present['meaning']}. "
                f"Future card: {future['name']} — {future['meaning']}. "
                f"Write a flowing, poetic reading that weaves all three cards together into one narrative arc. "
                f"Structure it as: one sentence about the past, two sentences about the present, one sentence about the future. "
                f"Speak directly to the seeker using 'you'. Do not use bullet points or headers. "
                f"Make it feel deeply mystical and personal."
            )
